...
import joblib
import time
from get_sorted_filenames import get_sorted_filenames
import cv2
...
lr_set = 0.01
num_epochs = 100
cnn_channel_nums_set = [16,32]
mlp_hidden_size_set = [32]
fusion_mlp_hidden_size = [64, 32, 16]
...
input_scaler.fit(df0.loc[:, est_vec])
if len(output_cols)==1:
    output_scaler.fit(df0.loc[:, output_cols].values.reshape(-1,1))
else:
    output_scaler.fit(df0.loc[:, output_cols])

df_train.loc[:, est_vec] = input_scaler.transform(df_train.loc[:, est_vec])
df_train.loc[:, output_cols] = output_scaler.transform(df_train.loc[:, output_cols])
df_val.loc[:, est_vec] = input_scaler.transform(df_val.loc[:, est_vec])
